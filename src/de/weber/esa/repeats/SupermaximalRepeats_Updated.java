...
public class SupermaximalRepeats_Updated {
    ...
    private List<Repeats> list = new ArrayList<>();

    public SupermaximalRepeats_Updated() {}
    ...
    public List<Repeats> computeSuperMaximalRepeats(final EnhancedSuffixArray esa,
        ...
        while (i < n) {
            ...
            if (esa.lcp.getCurrentLcpValue(j) > minimumLength) {
...
//                    this.list.add(new Maxima(esa.suffices[i], esa.suffices[j], esa.lcp.getCurrentLcpValue(j)));
                    Repeats.fillList(this.list, esa, i, j, esa.lcp.getCurrentLcpValue(j));
                }
            }
